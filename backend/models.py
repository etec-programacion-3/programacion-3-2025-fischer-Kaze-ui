...
from sqlalchemy import Column, Integer, String, Text, DECIMAL, DateTime, Boolean, ForeignKey, UniqueConstraint
...
# =====================================================================
# MODELOS PARA CARRITO (Issue 5)
# =====================================================================


class Carrito(Base):
    """
    Representa el carrito de compras persistente de un usuario.
    Relación 1:1 con Usuario (id_usuario es PK y FK).
    """
    ...
    # cascade="all, delete-orphan": Si se borra el carrito, se borran sus ítems.
    items = relationship("ItemCarrito", back_populates="carrito", cascade="all, delete-orphan")


class ItemCarrito(Base):
    """
    Representa un producto específico (y su cantidad) dentro de un Carrito.
    """
    __tablename__ = "items_carrito"
   
    id_item_carrito = Column(Integer, primary_key=True, autoincrement=True)
    # id_carrito es el id_usuario del carrito al que pertenece
    id_carrito = Column(Integer, ForeignKey('carrito.id_usuario'), nullable=False)
    id_producto = Column(Integer, ForeignKey('productos.id_producto'), nullable=False)
    cantidad = Column(Integer, nullable=False, default=1)
   
    # Restricción única: No puede haber dos filas con el mismo id_carrito y id_producto.
    # Esto asegura que un producto solo aparezca una vez por carrito.
    __table_args__ = (
        UniqueConstraint('id_carrito', 'id_producto', name='_carrito_producto_uc'),
    )
   
    # Relación N:1 con Carrito
